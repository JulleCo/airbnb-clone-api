...
module.exports = {
    signup: (request, response) => {
        ...
        if (passwordRegex.test(user.password) == false) {
            return response.status(400).json({
                'error': "Le mot de passe doit faire entre 6 et 20 caractères, et avoir au minimum : 1 chiffre, une lettre majuscule, une lettre minuscule."
            })
        }
        if (user.role !== ('host' || 'tourist')) {
            return response.status(400).json({
                'error': "Veuillez choisir votre mode d'utilisation"
            })
        }
        ...
        models.User.findOne({
                        ...
                        const newUser = models.User.create({
                            ...
                            .catch((error) => {
                                return response.status(500).json({
                                    'error': "Impossible d'ajouter l'utilisateur-ice"
                                })
                            })
                    })
                } else {
            ...
            .catch((error) => {
                return response.status(500).json({
                    'error': "Impossible de vérifier l'utilisateur-ice"
                })
            })
    },
            ...
            .then((isMatch) => {
                if (isMatch) {
                    ...
                    return response.status(404).json({
                        'error': "ce compte n'existe pas encore"
                    })
                }
            })
            .catch((error) => {
